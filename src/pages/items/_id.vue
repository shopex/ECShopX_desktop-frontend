...
<script>
import { resolveLazyLoadImg, hasValue } from '@/utils'
...
export default {
  ...
  computed: {
    breadcrumb() {
      if (!this.info) return []
      const { item_category_info } = this.info
      let ret = [
        {
          name: '首页',
          url: `/`
