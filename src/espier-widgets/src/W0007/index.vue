      ...
      <div class="widget-w0007-body widget-body" :style="widgetBodyStyle">
        ...
        <div :class="['all-category', !value.hover ? 'hover' : 'open']">
          <div
            ...
            :style="{ backgroundColor: '#FF5D02', color: '#FFFFFF', fontWeight:'800'}"
          >
            全部商品分类
          </div>
          <div class="category-list" :style="{ top: `${value.menuTop}px`, paddingTop: '16px'}">
            ...
            >
              <div
                ...
                :style="{
                  backgroundColor: btnHover != item.category_id ? getThemeColor : '#F5F6FA',
                  color: btnHover == item.category_id ? getThemeColor : value.wgtTextColor
                }"
              >
                <img class="category-main__btn__img" :src="item.image_url" width="16" height="16"/>

                <span class="category-main__btn_title">{{ item.category_name }}</span>
                <br />
                <span :class="sindex==0?'category-main__btn_subtitle__first':''" class="category-main__btn_subtitle" v-for="(sitem, sindex) in item.children">
                  {{sitem.category_name}}
                </span>
