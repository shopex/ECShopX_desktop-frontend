...
<template>
  <div class="sp-photo-zoom-pointitem pic-img">
    <div
      class="img-container"
...
export default {
  ...
  methods: {
    ...
    imgLoaded() {
      let imgInfo2 = this.$img.getBoundingClientRect()
      // IE浏览器兼容问题
      // if (JSON.stringify(this.imgInfo) != JSON.stringify(imgInfo2)) {
      if (this.imgInfo != imgInfo2) {
        this.imgInfo = imgInfo2
    ...
    mouseMove(e) {
      if (!this.hideZoom && this.imgLoadedFlag) {
        ...
        const { width, scale, selector, outShow, addWidth, outShowAutoScroll } = this
        ...
        // 边界处理
        if (x > 0 && x < 400 - width) {
          selector.bgLeft = addWidth - x * scale
        }
        if (y > 0 && y < 512 - width) {
